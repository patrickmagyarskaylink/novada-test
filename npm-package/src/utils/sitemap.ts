 ...
 */
function isSameSite(candidate: string, baseHostname: string): boolean {
  try {
 ...
 */
export function extractSitemapUrls(xml: string, out: string[], max: number, baseHostname: string): void {
  const matches = [...xml.matchAll(/<loc>\s*(.*?)\s*<\/loc>/gs)];
  for (const m of matches) {
    if (out.length >= max) break;
    const u = m[1].trim();
    if (u.startsWith("http") && isSameSite(u, baseHostname)) out.push(u);
  }
}
    ...
    if (typeof robotsResp.data === "string") {
      ...
      if (sitemapMatches) {
        for (const m of sitemapMatches) {
          ...
          if (u.startsWith("http") && isSameSite(u, baseHostname)) sitemapCandidates.unshift(u); // prefer robots.txt sitemap
        }
      }
    }
