...
function base64UrlDecodePermissive(input: string): string | null {
  if (!input || input.length > MAX_REDIRECT_PARAM_LENGTH) return null;
...
function decodeRedirectParamValue(value: string): string | null {
  if (value.length > MAX_REDIRECT_PARAM_LENGTH) return null;
  const stripped = value.replace(/^a1/, "");
  const decoded = base64UrlDecodePermissive(stripped);
  if (decoded && isHttpUrl(decoded)) return decoded;
  ...
  try {
    ...
    if (isHttpUrl(percentDecoded)) return percentDecoded;
