...
export class FileNoticeStore implements NoticeStore {
  async hasNoticed(): Promise<boolean> {
  ...
  async markNoticed(): Promise<void> {
    if (isVercel()) return;
    try {
      ...
      await fs.writeFile(
        flagPath(),
        JSON.stringify({ noticedAt: new Date().toISOString() }),
        "utf8",
      );
    } catch {
...
let defaultStore: NoticeStore | undefined;
function getDefaultStore(): NoticeStore {
  if (!defaultStore) defaultStore = new FileNoticeStore();
  return defaultStore;
}
 ...
 */
export async function maybeGetFirstRunNotice(store?: NoticeStore): Promise<string | null> {
  if (process.env[KILL_SWITCH_ENV]) return null;
  try {
