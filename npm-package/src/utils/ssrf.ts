...
function isBlockedIpv4(ip: string): boolean {
  ...
  if (!o) return false;
  const [a, b] = o;
  if (a === 0) return true;                       // 0.0.0.0/8  "this host" — routes to loopback
 ...
 */
export function assertUrlSafe(url: string, context = "fetch target"): void {
  if (!isUrlSafe(url)) {
    throw new Error(
      `Blocked ${context}: "${url}" resolves to a private/loopback/link-local host or a ` +
 ...
 */
export function safeLookup(hostname: string, options: object, cb: LookupCallback): void {
  ...
  dns.lookup(hostname, { ...opts, all: true }, (err, addresses) => {
    ...
    for (const a of list) {
      if (isBlockedIp(a.address)) {
        cb(
          new Error(
            `SSRF guard: "${hostname}" resolved to private/loopback/link-local address ${a.address} ` +
    ...
    if ((opts as dns.LookupAllOptions).all) {
      ...
      const first = list[0];
      if (!first) { cb(new Error(`SSRF guard: no address resolved for ${hostname}`), ""); return; }
