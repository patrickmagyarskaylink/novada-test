...
export const BrowserFlowParamsSchema = z.object({
  ...
  actions: z
    ...
    .describe(
      "Ordered sequence of browser actions to execute. Each action has a type and optional selector/value/delay."
    ),
  country: z
    .string()
    ...
    .default("")
    .describe(
      "Optional 2-letter ISO 3166-1 country code for geo-targeting the browser session (e.g. 'us', 'gb', 'de')."
    ),
  session_id: z
    .string()
    ...
    .optional()
    .describe(
      "Optional session ID for sticky sessions. When provided, the same browser session is reused across calls — preserving cookies, login state, and localStorage. Sessions expire after 10 minutes of inactivity."
    ),
});

export type BrowserFlowParams = z.infer<typeof BrowserFlowParamsSchema>;

export function validateBrowserFlowParams(
...
interface BrowserFlowApiResponse {
  code: number;
  msg?: string;
  data?: {
    session_id?: string;
 ...
 */
export async function novadaBrowserFlow(
  params: BrowserFlowParams,
  apiKey: string
): Promise<string> {
  const { url, actions, country, session_id } = params;

  let apiResponse: BrowserFlowApiResponse | undefined;
  ...
  try {
    ...
    apiResponse = resp.data as BrowserFlowApiResponse;
  } catch (err: unknown) {
    if (err instanceof AxiosError) {
      ...
      if (status === 401 || status === 403) {
        throw makeNovadaError(
          ...
          "Invalid NOVADA_API_KEY or insufficient permissions for Browser Flow API."
        );
      }
      ...
      if (status === 429) {
        throw makeNovadaError(
          ...
          "Browser Flow API rate limit exceeded."
        );
      }

      const serverMsg = (err.response?.data as BrowserFlowApiResponse)?.msg ?? err.message;

      if (status && status >= 500) {
        throw makeNovadaError(
          NovadaErrorCode.API_DOWN,
          `Browser Flow API error (HTTP ${status}): ${serverMsg}`
        );
      }

      throw makeNovadaError(
        NovadaErrorCode.URL_UNREACHABLE,
        `Browser Flow API unreachable (HTTP ${status ?? "network error"}): ${serverMsg}`
      );
    }
  ...
  const actionResults: ActionResult[] = resultData?.results ?? resultData?.actions ?? [];
...
function formatSuccessResponse(
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (r.error) {
      ...
      const content =
        r.content.length > 10000 ? r.content.slice(0, 10000) + "\n<!-- truncated -->" : r.content;
      lines.push(content);
    } else {
  ...
  if (sessionId) {
    lines.push(
      `- Session active: session_id="${sessionId}" — reuse this ID in subsequent novada_browser_flow calls to maintain state.`
    );
    ...
    lines.push(
      `- Pass session_id to maintain login state and cookies across multiple novada_browser_flow calls.`
    );
  }
  lines.push(`- For CDP-based interactive automation with more action types, use novada_browser instead.`);
  if (failed > 0) {
    lines.push(
      `- ${failed} action(s) failed. If the issue persists, use novada_browser as a fallback — it supports more action types and richer error detail.`
    );
  }
...
function formatEndpointUnavailable(url: string, actions: BrowserFlowAction[]): string {
  return [
