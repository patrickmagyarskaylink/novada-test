...
function isMarkdownPassthroughContentType(contentType: string, body: string): boolean {
...
async function fetchSitePage(
...
): Promise<{ body: string; isMarkdown: boolean } | null> {
  ...
  try {
    ...
    let contentType = String((resp.headers as Record<string, string> | undefined)?.["content-type"] ?? "");
    ...
    if (renderMode === "auto" && detectJsHeavyContent(body)) {
      try {
        ...
        if (typeof r.data === "string") {
          body = r.data;
          contentType = String((r.headers as Record<string, string> | undefined)?.["content-type"] ?? "");
          if (isMarkdownPassthroughContentType(contentType, body)) {
            return { body, isMarkdown: true };
          }
        }
