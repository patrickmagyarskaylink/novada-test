...
const WALMART_OPERATION_PARAMS_DOC: Record<WalmartOperation, string> = {
...
const WALMART_OPERATION_CONFIGS: Record<WalmartOperation, PlatformOperationConfig> = Object.fromEntries(
  (Object.keys(WALMART_OPERATIONS) as WalmartOperation[]).map((name) => [
    name,
    { scraperId: WALMART_OPERATIONS[name], paramsDoc: WALMART_OPERATION_PARAMS_DOC[name] },
  ]),
) as Record<WalmartOperation, PlatformOperationConfig>;

/** Walmart's declarative platform-scraper config — the factory's sole input for this tool. */
export const WALMART_SCRAPER_CONFIG: PlatformScraperConfig<WalmartOperation> = {
  platform: "walmart.com",
  platformLabel: "Walmart",
  toolName: "novada_scrape_walmart",
  category: "Scraping & Verification",
  registryDescription:
    "Extract structured Walmart data (product details by keyword/category URL/SKU/zip code/URL) via a closed, typed operation enum — 5 verified-working operations; same engine and output formats as novada_scrape, pinned to platform=walmart.com",
  operations: WALMART_OPERATION_CONFIGS,
  paramsFieldDoc:
    "Operation-specific parameters for the selected `operation`. E.g. { domain: \"https://www.walmart.com/\", keyword: \"shoes\" } for " +
...
export function validateScrapeWalmartParams(args: Record<string, unknown> | undefined): ScrapeWalmartParams {
 ...
 * duplicated here.
 */
export async function novadaScrapeWalmart(params: ScrapeWalmartParams, apiKey: string): Promise<string> {
