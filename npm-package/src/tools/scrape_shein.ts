...
export const SHEIN_SCRAPER_CONFIG: PlatformScraperConfig<SheinOperation> = {
  ...
  description: {
    core:
      "Extract structured SHEIN product data — product details by product ID or product URL — via a closed, typed `operation` enum (same engine as novada_scrape, pinned to platform=\"shein.com\").",
    useWhen: [
    ...
    returns:
      "Structured product records (title, price, rating, images, etc.) in the chosen output format — same rendering as novada_scrape.",
    operationsNote:
      "2 verified-working SHEIN operations: product lookup by product ID or by product URL. 3 known backend_broken SHEIN operations — product-list by keyword, by category ID, and by category URL, all failing with a 60s+ submit-endpoint hang — are intentionally NOT in this enum; this tool rejects them before any backend call, unlike novada_scrape(platform=\"shein.com\", ...), which still forwards them with a warning.",
  },
};
