...
const GOOGLE_OPERATION_PARAMS_DOC: Record<GoogleOperation, string> = {
...
const GOOGLE_OPERATION_CONFIGS: Record<GoogleOperation, PlatformOperationConfig> = Object.fromEntries(
  (Object.keys(GOOGLE_OPERATIONS) as GoogleOperation[]).map((name) => [
    name,
    { scraperId: GOOGLE_OPERATIONS[name], paramsDoc: GOOGLE_OPERATION_PARAMS_DOC[name] },
  ]),
) as Record<GoogleOperation, PlatformOperationConfig>;

/** Google's declarative platform-scraper config — the factory's sole input for this tool. */
export const GOOGLE_SCRAPER_CONFIG: PlatformScraperConfig<GoogleOperation> = {
  platform: "google.com",
  platformLabel: "Google",
  toolName: "novada_scrape_google",
  category: "Scraping & Verification",
  registryDescription:
    "Extract structured Google SERP data (web search, AI Mode, Maps details/reviews, Shopping, Jobs, Hotels, Videos) via a closed, typed operation enum — 13 verified-working operations; same engine and output formats as novada_scrape, pinned to platform=google.com",
...
export function validateScrapeGoogleParams(args: Record<string, unknown> | undefined): ScrapeGoogleParams {
 ...
 * duplicated here.
 */
export async function novadaScrapeGoogle(params: ScrapeGoogleParams, apiKey: string): Promise<string> {
