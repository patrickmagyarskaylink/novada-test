 ...
 */
export const AUTH_FREE_TOOLS: ReadonlySet<string> = new Set([
  "novada_setup",
  "novada_discover",
 ...
 */
export const UNAUTH_BASIC_TIER_TOOLS: ReadonlySet<string> = new Set([
  "novada_extract",
]);

/** Class lookup — everything not in the two tables above requires a key. */
export function toolAuthClass(name: string): ToolAuthClass {
  if (AUTH_FREE_TOOLS.has(name)) return "auth_free";
  if (UNAUTH_BASIC_TIER_TOOLS.has(name)) return "unauth_basic_tier";
  return "key_required";
}
 ...
 */
export const ESCALATION_RENDER_MODES: ReadonlySet<string> = new Set(["render", "js", "browser"]);

/** True when a call to an unauth-tier tool EXPLICITLY requests key-gated escalation. */
export function isEscalationRequest(name: string, args?: Record<string, unknown>): boolean {
  if (toolAuthClass(name) !== "unauth_basic_tier") return false;
 ...
 */
export function decideKeyGate(
  name: string,
  opts: { hasApiKey: boolean; kr6Bypass: boolean; args?: Record<string, unknown> }
): KeyGateDecision {
  if (opts.hasApiKey || opts.kr6Bypass) return { kind: "allow" };
  const cls = toolAuthClass(name);
  if (cls === "auth_free") return { kind: "allow" };
...
// ─── Unauthenticated-tier call context (read by utils/output.ts) ─────────────

interface GateCallContext {
  unauthenticatedTier: boolean;
}

const gateContextStorage = new AsyncLocalStorage<GateCallContext>();
 ...
 */
export function isUnauthenticatedTierCall(): boolean {
...
export async function runUnauthenticatedTier<T>(fn: () => Promise<T>): Promise<T> {
 ...
 */
export const UNAUTH_TIER_DISCLOSURE =
  "Notice [unauthenticated tier]: this basic direct fetch ran WITHOUT a valid NOVADA_API_KEY. " +
