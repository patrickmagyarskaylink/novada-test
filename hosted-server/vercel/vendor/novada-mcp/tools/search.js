 ...
 */
export async function withSingleSerpRetry(op, delayMs = SEARCH_SUBMIT_RETRY_DELAY_MS) {
    try {
        return await op();
    }
    catch (err) {
...
async function submitBingSearch(apiKey, query) {
    for (let attempt = 0; attempt < 3; attempt++) {
        ...
        if (body.code !== 0) {
            ...
            throw makeNovadaError(NovadaErrorCode.API_DOWN, `Bing search error (code ${body.code}): ${sanitizeServerMsg(body.msg ?? "unknown")}`);
        }
...
export async function pollSearchResult(apiKey, taskId) {
    ...
    while (Date.now() < deadline) {
        ...
        if (body !== null && typeof body === "object" && !Array.isArray(body)) {
            ...
            throw makeNovadaError(NovadaErrorCode.API_DOWN, `Scraper download error (code ${bObj.code ?? "?"}): ${sanitizeServerMsg(rawDetail)}`);
        }
        // H4: never embed the raw upstream body verbatim.
        throw makeNovadaError(NovadaErrorCode.API_DOWN, `Unexpected scraper download response: ${sanitizeServerMsg(JSON.stringify(body)).slice(0, 200)}`);
    }
    throw makeNovadaError(NovadaErrorCode.API_DOWN, `Scraper search task ${taskId} timed out after ${TIMEOUTS.SEARCH_TOTAL_CEILING / 1000}s.`);
}
...
export async function novadaSearch(params, apiKey, options) {
    ...
    try {
        {
            ...
            const submitted = await withSingleSerpRetry(() => submitSearchScrapeTask(apiKey, engineCfg.scraper_name, engineCfg.scraper_id, effectiveQuery, overFetchNum, engineCfg.query_param, engineCfg.supports_num, {
        ...
        const isEntitlement = status === 401 || status === 402 || status === 403 ||
        ...
        throw makeNovadaError(NovadaErrorCode.API_DOWN, `Search temporarily unavailable — upstream issue: ${sanitizeServerMsg(msg).slice(0, 200)}`);
    }
