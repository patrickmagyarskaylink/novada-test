...
const DUCKDUCKGO_OPERATION_PARAMS_DOC = {
...
const DUCKDUCKGO_OPERATION_CONFIGS = Object.fromEntries(Object.keys(DUCKDUCKGO_OPERATIONS).map((name) => [
    name,
...
export const DUCKDUCKGO_SCRAPER_CONFIG = {
    platform: "duckduckgo.com",
    platformLabel: "DuckDuckGo",
    toolName: "novada_scrape_duckduckgo",
    category: "Scraping & Verification",
    registryDescription: "Extract structured DuckDuckGo web search SERP data via a closed, typed operation enum — 1 verified-working operation; same engine and output formats as novada_scrape, pinned to platform=duckduckgo.com",
    operations: DUCKDUCKGO_OPERATION_CONFIGS,
    paramsFieldDoc: "Operation-specific parameters for the selected `operation`. E.g. { q: \"wireless earbuds\", kl: \"us-en\" } for web_search.",
    description: {
        core: "Extract structured DuckDuckGo web search SERP data via a closed, typed `operation` enum (same engine as novada_scrape, pinned to platform=\"duckduckgo.com\"). Returns the raw organic-results SERP, with region/time/adult-content filters novada_search does not expose, not a generated answer.",
        useWhen: [
        ...
        operationsNote: "1 verified-working DuckDuckGo operation (web search). The single duckduckgo.com catalog operation is currently status:\"ok\".",
    },
};
...
export function validateScrapeDuckduckgoParams(args) {
 ...
 */
export async function novadaScrapeDuckduckgo(params, apiKey) {
    return DUCKDUCKGO_SCRAPER_TOOL.handler(params, apiKey);
}
//# sourceMappingURL=scrape_duckduckgo.js.map