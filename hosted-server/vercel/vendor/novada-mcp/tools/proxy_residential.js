...
export function validateProxyResidentialParams(args) {
    return ProxyResidentialParamsSchema.parse(args ?? {});
}
// ─── Username Builder ─────────────────────────────────────────────────────────
function buildResidentialUsername(user, params) {
    const parts = [user];
    // Residential zone
    parts.push("zone-res");
    if (params.country)
    ...
    if (params.city)
        parts.push(`city-${params.city.toLowerCase().replace(/\s+/g, "")}`);
 ...
 */
export async function novadaProxyResidential(params) {
...
//# sourceMappingURL=proxy_residential.js.map