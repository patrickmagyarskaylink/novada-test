...
export const FLOW_LEDGER_PRODUCTS = new Set(FLOW_LEDGER_ROWS.map((r) => r.key));
/**
 ...
 */
export async function preflightFlowLedger(product, billingApiKey) {
    const row = FLOW_LEDGER_ROWS.find((r) => r.key === product);
    if (!row)
        return null;
    const base = { product: row.key, label: row.label, ledger: row.path };
    try {
        const raw = await withTimeout(novadaPlanBalanceAll({ products: [row.key] }, billingApiKey), PREFLIGHT_TIMEOUT_MS);
        const parsed = JSON.parse(raw);
        const entry = parsed?.per_product?.[row.key];
        if (!entry)
            return { ...base, status: "unknown", detail: "ledger response carried no entry for this product" };
        if (entry.status === "error") {
            if (entry.unavailable === true)
                return { ...base, status: "unavailable", detail: entry.error };
        ...
        if ((entry.exhausted ?? derived.exhausted) === true)
            return { ...evidence, status: "exhausted" };
 ...
 */
export async function assertFlowLedgerActive(product, creds, toolHint = `novada_proxy(type="${product}")`) {
    const row = FLOW_LEDGER_ROWS.find((r) => r.key === product);
    if (!row)
        return null;
    if (creds.source === "direct") {
        ...
        return {
            ...
            ledger: row.path,
            status: "unknown",
            detail: "billing account unknown — credentials were supplied directly (env/SDK), ledger not consulted",
        };
    }
    ...
    if (pf.status === "expired" || pf.status === "exhausted" || pf.status === "unavailable") {
        ...
        throw new NovadaError({
            ...
            message: `Your ${pf.label} proxy plan is ${reason} — Novada will not route traffic for it. ` +
            ...
            agent_instruction: `Do NOT use ${toolHint} right now — the ${pf.label} flow ledger (${pf.ledger}) shows ` +
...
//# sourceMappingURL=proxy_preflight.js.map