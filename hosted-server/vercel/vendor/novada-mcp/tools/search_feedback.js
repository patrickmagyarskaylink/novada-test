...
export const SearchFeedbackParamsSchema = z.object({
    search_id: z
        ...
        .regex(/^[a-zA-Z0-9_\-:.]+$/, "search_id must be alphanumeric with underscores, hyphens, dots, or colons only")
        .describe("The id of the search this feedback is about. Use the search_id returned by a prior novada_search call (or any stable identifier you used for the query)."),
    query: z
        .string()
    ...
    useful_urls: z
        ...
        .optional()
        .default([])
        .describe("URLs from the result set that were actually useful. Drives future ranking — list the results you clicked or cited. Max 50."),
    rating: z
        .enum(["good", "ok", "bad"])
        .describe("Overall quality of the result set. 'good': relevant + sufficient. 'ok': partially useful. 'bad': irrelevant or missing what you needed."),
    note: z
        .string()
        .max(2000)
        .optional()
    ...
    format: z
        .enum(["markdown", "json"])
        .default("markdown")
        .describe("Output format. 'markdown' (default): human-readable confirmation. 'json': structured object for programmatic agent use."),
});
export function validateSearchFeedbackParams(args) {
 ...
 */
export async function novadaSearchFeedback(params) {
    const { entry, submissions_for_search } = storeFeedback(params);
    const agentInstruction = "Feedback recorded in-memory for this session — it will bias future ranking. No need to re-submit the same search_id unless your assessment changes.";
...
//# sourceMappingURL=search_feedback.js.map