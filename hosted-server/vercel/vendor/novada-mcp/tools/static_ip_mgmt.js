...
// ─── Helpers ─────────────────────────────────────────────────────────────────
function assertDefined(value, field, action) {
    if (value === undefined) {
...
// ── open ─────────────────────────────────────────────────────────────────────
async function handleOpen(params, apiKey) {
    ...
    const data = await devApiPost("/v1/static_house/open", body, { apiKey });
    return JSON.stringify({
        status: "opened",
        data,
        agent_instruction: "Static IPs purchased. Use novada_static_ip_mgmt with action='list' to confirm they appear.",
    }, null, 2);
}
// ── renew ────────────────────────────────────────────────────────────────────
async function handleRenew(params, apiKey) {
    ...
    const data = await devApiPost("/v1/static_house/renew", body, { apiKey });
    return JSON.stringify({
        status: "renewed",
        data,
        agent_instruction: "Static IPs renewed. Use novada_static_ip_mgmt with action='list' to verify updated expiry dates.",
    }, null, 2);
}
// ── export ───────────────────────────────────────────────────────────────────
async function handleExport(params, apiKey) {
    const body = {
    ...
    const data = await devApiPost("/v1/static_house/export", body, { apiKey });
    return JSON.stringify({
        status: "ok",
        data,
        agent_instruction: "Export of static ISP IPs returned. Data includes all IPs matching the applied filters.",
    }, null, 2);
}
// ── list ─────────────────────────────────────────────────────────────────────
async function handleList(params, apiKey) {
    ...
    const data = await devApiPost("/v1/static_house/list", body, { apiKey });
    return JSON.stringify({
        status: "ok",
        data,
        agent_instruction: "Paginated list of static ISP IPs. Use 'export' action for full unfiltered dump. Use 'open' to purchase new IPs or 'renew' to extend existing ones.",
    }, null, 2);
}
//# sourceMappingURL=static_ip_mgmt.js.map