...
export const ProxyAccountCreateParamsSchema = z
    ...
    approval_token: z
        ...
        .min(1)
        .optional()
        .describe("Two-step approval token. Omit on the first call to receive a preview + a fresh approval_token (valid 10 minutes). Re-call with the EXACT SAME parameters plus this field set to that token to execute. Never invent a value — an invented or stale token is rejected."),
})
    .strict();
...
export async function novadaProxyAccountCreate(params, apiKey) {
    const gate = evaluateApprovalGate(params, "create_proxy_sub_account");
    if (!gate.authorized) {
    ...
    return JSON.stringify({
        status: "created",
        data: maskedData,
        agent_instruction: "Sub-account created on the user's Novada plan. Use novada_proxy_account_list (with the same `product` code) to confirm it appears. The password is masked in this response for security — the user set it and already has it.",
    }, null, 2);
}
