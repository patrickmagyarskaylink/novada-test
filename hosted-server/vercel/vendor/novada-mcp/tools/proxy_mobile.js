...
export function validateProxyMobileParams(args) {
    return ProxyMobileParamsSchema.parse(args ?? {});
}
// ─── Username Builder ─────────────────────────────────────────────────────────
function buildMobileUsername(user, params) {
    const parts = [user];
    // Mobile zone
    parts.push("zone-mob");
    if (params.country)
    ...
    if (params.carrier)
        parts.push(`carrier-${params.carrier.toLowerCase().replace(/\s+/g, "")}`);
...
export async function novadaProxyMobile(params) {
    ...
    if (params.country)
        targetingParts.push(params.country.toUpperCase());
    if (params.carrier)
        targetingParts.push(`carrier: ${params.carrier}`);
...
//# sourceMappingURL=proxy_mobile.js.map