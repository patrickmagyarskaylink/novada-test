...
export const AMAZON_SCRAPER_CONFIG = {
    ...
    description: {
        core: "Extract structured Amazon data — product details, reviews, seller info, bestseller lists, category/brand listings — via a closed, typed `operation` enum (same engine as novada_scrape, pinned to platform=\"amazon.com\").",
        useWhen: [
        ...
        returns: "Structured product/review/seller records (title, price, rating, asin, availability, etc.) in the chosen output format — same rendering as novada_scrape.",
        operationsNote: "10 verified-working Amazon operations. 3 known backend-broken ones are excluded from this enum (rejected client-side before any backend call) — unlike novada_scrape(platform=\"amazon.com\", ...), which still forwards them with a warning.",
    },
};
