 ...
 */
export function isVerifySupportedRuntime() {
 ...
 */
export function sanitizeEchoField(value, kind = "text") {
 ...
 */
export function verifyProxyExit(target, timeoutMs = DEFAULT_ECHO_TIMEOUT_MS) {
    ...
    return new Promise((resolve) => {
        ...
        const fail = (failure_class, detail, http_status) => settle({
            verified: false,
            failure_class,
            detail,
        ...
        const req = http.request({
            ...
            if (status === 402) {
                res.resume();
                fail("payment_required", "gateway accepted the credentials but refused to route traffic (HTTP 402 Payment Required) — " +
                    "the plan's balance is exhausted or the plan is expired. Outside this check the failure is " +
                    "visible only as curl exit code 56.", 402);
                return;
            }
            if (status === 407) {
                res.resume();
                fail("auth_failed", "proxy rejected the credentials (HTTP 407 Proxy Authentication Required) — username/password " +
                    "invalid for this gateway.", 407);
                return;
            }
            if (status !== 200) {
                res.resume();
 ...
 */
export function pickProxyListEntry(envValue) {
    if (!envValue)
        return null;
...
//# sourceMappingURL=proxy_verify.js.map