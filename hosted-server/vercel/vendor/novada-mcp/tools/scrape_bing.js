...
const BING_OPERATION_PARAMS_DOC = {
...
const BING_OPERATION_CONFIGS = Object.fromEntries(Object.keys(BING_OPERATIONS).map((name) => [
    name,
...
export const BING_SCRAPER_CONFIG = {
    platform: "bing.com",
    platformLabel: "Bing",
    toolName: "novada_scrape_bing",
    category: "Scraping & Verification",
    registryDescription: "Extract structured Bing SERP data (web search, videos, news, shopping) via a closed, typed operation enum — 4 verified-working operations; same engine and output formats as novada_scrape, pinned to platform=bing.com",
    ...
    description: {
        core: "Extract structured Bing SERP data — organic web search, video results, news results, shopping listings — via a closed, typed `operation` enum (same engine as novada_scrape, pinned to platform=\"bing.com\"). Bing is NOT a selectable engine on novada_search — this is the only intentional way to query Bing specifically.",
        useWhen: [
        ...
        returns: "Structured SERP records per operation — organic results (title/url/snippet/rank), video results, news results (with date), shopping listings (price/rating) — in the chosen output format, same rendering as novada_scrape.",
        operationsNote: "4 verified-working Bing operations: web search, videos, news, and shopping. Every bing.com catalog operation is currently status:\"ok\" — none are excluded for being backend_broken.",
    },
};
...
export function validateScrapeBingParams(args) {
 ...
 * duplicated here.
 */
export async function novadaScrapeBing(params, apiKey) {
    return BING_SCRAPER_TOOL.handler(params, apiKey);
}
//# sourceMappingURL=scrape_bing.js.map