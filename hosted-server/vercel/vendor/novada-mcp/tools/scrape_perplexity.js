...
const PERPLEXITY_OPERATION_PARAMS_DOC = {
...
const PERPLEXITY_OPERATION_CONFIGS = Object.fromEntries(Object.keys(PERPLEXITY_OPERATIONS).map((name) => [
    name,
...
export const PERPLEXITY_SCRAPER_CONFIG = {
    platform: "perplexity.ai",
    platformLabel: "Perplexity AI",
    toolName: "novada_scrape_perplexity",
    category: "Scraping & Verification",
    registryDescription: "Extract Perplexity AI's own generated answer for a query (by URL or raw search term) via a closed, typed operation enum — 2 verified-working operations; same engine and output formats as novada_scrape, pinned to platform=perplexity.ai",
    operations: PERPLEXITY_OPERATION_CONFIGS,
    paramsFieldDoc: "Operation-specific parameters for the selected `operation`. E.g. { url: \"https://www.perplexity.ai/?q=apple\" } for " +
        "answer_by_url, { search_terms: \"Today's weather\" } for answer_by_search_term.",
    description: {
        core: "Extract the actual AI-generated answer Perplexity returns for a query — by a Perplexity query URL or a raw search term — via a closed, typed `operation` enum (same engine as novada_scrape, pinned to platform=\"perplexity.ai\"). Scrapes Perplexity's own live-rendered answer page (the model's actual response text) — NOT a web search results list and NOT an indexed-page brand-mention scan.",
        useWhen: [
        ...
        returns: "Perplexity's rendered answer content (the text of its generated response) in the chosen output format — same rendering as novada_scrape.",
        operationsNote: "2 verified-working Perplexity operations: answer by a perplexity.ai query URL, or answer by a raw search term. Every perplexity.ai catalog operation is currently status:\"ok\" — unlike its sibling chatgpt.com, whose only 2 catalog operations are BOTH backend_broken (verified 2026-07-13, \"submit hangs >120s\") — which is why no novada_scrape_chatgpt tool exists.",
    },
};
...
export function validateScrapePerplexityParams(args) {
 ...
 */
export async function novadaScrapePerplexity(params, apiKey) {
    return PERPLEXITY_SCRAPER_TOOL.handler(params, apiKey);
}
//# sourceMappingURL=scrape_perplexity.js.map