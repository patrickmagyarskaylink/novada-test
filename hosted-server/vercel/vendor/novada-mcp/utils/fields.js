 ...
 */
function isDescriptionBoilerplate(value) {
    const trimmed = value.trim();
    if (DESCRIPTION_BOILERPLATE_RE.test(trimmed))
        return true;
 ...
 */
const CANONICAL_TO_GERMAN = new Map();
 ...
 */
function labelMatchScoreWithGerman(label, canonical) {
    ...
    if (mappedCanonical === canonical)
        return 2; // treat like startsWith match
    return 0;
}
/**
...
function labelMatchScore(label, field) {
    ...
    if (Math.min(l.length, f.length) < 3)
        return 0;
 ...
 */
export function matchHeadingSectionWithReason(text, field) {
    const escapedField = field.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const lines = text.split("\n");
    let inSection = false;
    const sectionLines = [];
...
function collectDomCandidates($) {
    ...
    const considerTableRow = (rawLabel, rawValue) => {
    ...
    const considerDlRow = (rawLabel, rawValue) => {
        const label = rawLabel.trim();
        const value = rawValue.trim();
        if (!label || !value)
            return;
    ...
    for (const sel of HERO_ATTR_SELECTORS) {
        const el = $(sel).first();
        if (el.length && !attrHits.has(sel))
            attrHits.set(sel, el.text().trim());
    }
...
function extractFromInfobox(cand, fieldName) {
...
function extractFromTableHeaders(cand, fieldName) {
 ...
 */
function extractFromLabelValueRows(cand, fieldName) {
    const best = { score: 0, value: null };
...
function extractFromMicrodata(cand, fieldName) {
...
function isCartContext(context) {
    if (!context)
        return false;
 ...
 */
function extractFromAdjacentPairs(cand, canonical) {
    ...
    for (const word of hint.classWords) {
        for (const el of cand.classedEls) {
            if (!classTokenHasWord(el.className, word))
                continue;
            const v = accept(el.text);
            if (!v)
                continue;
            if (guardCart && (isCartContext(el.context) || looksLikeZeroTotal(v)))
                continue;
            return v;
        }
    }
 ...
 */
function findCartTotalPriceOnly(cand) {
    for (const el of cand.classedEls) {
        if (!classTokenHasWord(el.className, "price"))
            continue;
        const val = el.text.trim();
        if (!val || val.length > 60 || !looksLikeStatValue(val))
            continue;
        if (isCartContext(el.context) || looksLikeZeroTotal(val))
            return val.slice(0, 200);
    }
    return null;
}
 ...
 */
function applyConfidenceGate(r) {
    if (r.value === null || r.source === "unresolved")
        return r;
    if (r.confidence >= FIELD_CONFIDENCE_FLOOR)
        return r;
    return {
        field: r.field,
        value: null,
...
function resolvedWithWarning(field, value, source, attempted, warning) {
...
preloaded$) {
    ...
    function resolveField(field) {
        ...
        const skipAsCartZero = (v) => isPriceField && looksLikeZeroTotal(v);
            ...
            if (value && !skipAsCartZero(value)) {
                if (isDescriptionField && isDescriptionBoilerplate(value)) {
                    ...
                    if (isDescriptionField) {
                        ...
                        return resolvedWithWarning(field, value, "pattern", attempted, `'${field}' resolved from a regex pattern match, not a structured meta source. The value may not be the canonical page description. Consider retrying with render="render" or checking the HTML <meta name="description"> tag.`);
                    }
        ...
        if (near && !skipAsCartZero(near))
            return resolved(field, near, "proximity", attempted);
