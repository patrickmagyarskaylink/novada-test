 ...
 */
export const APPROVAL_AGENT_INSTRUCTION = "This action is irreversible and requires two steps. Call again WITHOUT " +
...
let localProcessSecret;
function getApprovalSecret(action) {
    if (isHostedEnvironment()) {
        ...
        if (!fromEnv) {
            ...
            throw new NovadaError({
                ...
                message: `"${action}" cannot be authorized on this hosted deployment: ` +
                    `NOVADA_APPROVAL_SECRET is not configured.`,
                agent_instruction: "This write action is unavailable until the operator sets " +
...
// ─── Sign / verify ──────────────────────────────────────────────────────────
function sign(secret, canonicalPayload, exp) {
...
function rejection(action, reason) {
 ...
 */
export function evaluateApprovalGate(params, action) {
    ...
    if (typeof approval_token === "string" && approval_token.length > 0) {
        ...
        throw rejection(action, "approval_token is missing, invalid, expired, or does not match the supplied parameters.");
    }
