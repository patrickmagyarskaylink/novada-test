 ...
 */
export function sanitizeSlug(s, maxLen = 80) {
 ...
 */
export async function resolveSiteCopyDir(domain, project) {
 ...
 */
export function safeSiteCopyFilePath(dir, slug, ext = "md") {
 ...
 */
async function getOutputDir(topic, project) {
    const today = new Date().toISOString().split("T")[0]; // YYYY-MM-DD
    let dir = join(homedir(), "Downloads", "novada-mcp", today);
    if (project)
        dir = join(dir, sanitize(project, 30));
    if (topic)
        dir = join(dir, topic);
